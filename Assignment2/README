...
The way this is secure is that any kind of file that has to be sent via email - 
Text, Image or Video can be encrypted by the sender and then decrypted by the receiver.
...
-> For symmetric encryption, AES-256 has been used in Cipher Block Chaining (CBC) mode. 256 bit key is used, 
   since a lower key length will make an adversary’s job much easier in cracking.
...
-> RSA has also been used for signing. 
   For signing, SHA-256 is used as the hashing algorithm because of greater resistance to collisions when compared to SHA1 or MD5.
